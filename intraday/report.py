...
def duration(
    date1: Union[Real, datetime, date, Any],
    ...
    if convention in {"actual/365", "actual_365", "actual365"}:
        ...
        assert (
            hasattr(date1, "year")
            and hasattr(date1, "month")
            and hasattr(date1, "day")
        )
        assert (
            hasattr(date2, "year")
            and hasattr(date2, "month")
            and hasattr(date2, "day")
        )
...
class Report(object):
    ...
    def add(self, record: Record):
        ...
        if d != 0:
            ...
            self.sortino_downside_returns_annual.append(
                min(0, roiann - self.risk_free_rate)
            )
        ...
        if self.max_drawdown < self.max_net_profit - self.net_profit:
            ...
            self.max_drawdown_percent = self.max_drawdown / (
                self.initial_balance + self.max_net_profit
            )
    ...
    def roi_mean(self):
        return (
            np.mean(self.returns_on_investments)
            if len(self.returns_on_investments) > 0
            else None
        )
    ...
    def roi_std(self):
        return (
            np.std(self.returns_on_investments)
            if len(self.returns_on_investments) > 0
            else 0
        )
    ...
    def roi_tdd(self):
        return (
            np.sqrt(np.mean(np.power(self.sortino_downside_returns, 2)))
            if len(self.sortino_downside_returns) > 0
            else 0
        )
    ...
    def roiann_mean(self):
        return (
            np.mean(self.returns_on_investments_annual)
            if len(self.returns_on_investments_annual) > 0
            else None
        )
    ...
    def roiann_std(self):
        return (
            np.std(self.returns_on_investments_annual)
            if len(self.returns_on_investments_annual) > 0
            else 0
        )
    ...
    def roiann_tdd(self):
        return (
            np.sqrt(np.mean(np.power(self.sortino_downside_returns_annual, 2)))
            if len(self.sortino_downside_returns_annual) > 0
            else 0
        )
    ...
    def sortino_ratio(self):
        ...
        return (
            ...
            if roi_mean is not None and roi_tdd != 0
            else None
        )
    ...
    def sortino_ratio_annual(self):
        ...
        return (
            ...
            if roiann_mean is not None and roiann_tdd != 0
            else None
        )
    ...
    def sharpe_ratio(self):
        ...
        return (
            roi_mean / roi_std
            if roi_mean is not None and roi_std != 0
            else None
        )
    ...
    def sharpe_ratio_annual(self):
        ...
        return (
            roiann_mean / roiann_std
            if roiann_mean is not None and roiann_std != 0
            else None
        )
    ...
    def recovery_factor(self):
        ...
        max_drawdown_percent = self.max_drawdown_percent
        return (
            rorann / (max_drawdown_percent + 1e-08)
            if rorann and max_drawdown_percent
            else None
        )
    ...
    def average_long_trade_result(self):
        return (
            self.long_result / self.n_trades_long
            if self.n_trades_long > 0
            else None
        )
    ...
    def average_short_trade_result(self):
        return (
            self.short_result / self.n_trades_short
            if self.n_trades_short > 0
            else None
        )
    ...
    def average_win_trade_result(self):
        return (
            self.gross_profit / self.n_trades_win
            if self.n_trades_win > 0
            else None
        )
    ...
    def average_loss_trade_result(self):
        return (
            self.gross_loss / self.n_trades_loss
            if self.n_trades_loss > 0
            else None
        )
    ...
    def average_trade_duration(self):
        return (
            self.trades_duration / self.n_trades if self.n_trades > 0 else None
        )

    @property
    def average_long_trade_duration(self):
        return (
            self.trades_duration_long / self.n_trades_long
            if self.n_trades_long > 0
            else None
        )
    ...
    def average_short_trade_duration(self):
        return (
            self.trades_duration_short / self.n_trades_short
            if self.n_trades_short > 0
            else None
        )
    ...
    def average_win_trade_duration(self):
        return (
            self.trades_duration_win / self.n_trades_win
            if self.n_trades_win > 0
            else None
        )
    ...
    def average_loss_trade_duration(self):
        return (
            self.trades_duration_loss / self.n_trades_loss
            if self.n_trades_loss > 0
            else None
        )

    def __repr(self):
        return f"{self.__class__.__name__}(initial_balance={self.initial_balance}, risk_free_rate={self.risk_free_rate}, convention={self.convention})"
