...
class Exchange:
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(agent_order_delay, Real):
            ...
            self.agent_order_delay = timedelta(
                seconds=float(agent_order_delay)
            )
            ...
            raise ValueError(
                "agent_order_delay should be either real number of seconds or a positive timedelta object"
            )
        if isinstance(broker_order_delay, Real):
            assert broker_order_delay > 0
            self.broker_order_delay = timedelta(
                seconds=float(broker_order_delay)
            )
            ...
            raise ValueError(
                "broker_order_delay should be either real number of seconds or a positive timedelta object"
            )
    ...
    def add_order(
        ...
        if (
            self.last_trade is not None
            and order.time_init <= self.last_trade.datetime
        ):
            raise ValueError("Cant add order in past time!")
        if (
            self.price_step is not None
            and hasattr(order, "price")
            and -math.inf < order.price < math.inf
        ):
            price = (
                math.floor(0.5 + order.price / self.price_step)
                * self.price_step
            )
        ...
        if (
            self.next_order_time is None
            or self.next_order_time > order.time_init
        ):
    ...
    def process_trade(self, trade: Union[Trade, TradeOI]):
        ...
        if (
            self.last_trade is not None
            and self.last_trade.operation != trade.operation
        ):
            spread = abs(self.last_trade.price - trade.price)
            self.spread_mean = (1 - self.ema_factor) * (
                self.spread_mean or spread
            ) + self.ema_factor * spread
            spread_var = (spread - self.spread_mean) ** 2
            self.spread_var = (1 - self.ema_factor) * (
                self.spread_var or spread_var
            ) + self.ema_factor * spread_var
        last_trade = self.last_trade
        self.last_trade = trade
        if (
            self.next_order_time is None
            or trade_datetime < self.next_order_time
        ...
        for id, order in orders.items():
            ...
            if trade_datetime < order.time_init:
                continue
            if (
                not self.instant_balance_update
                and order.account not in updated_accounts
            ):
                order.account.update_balance(price)
                updated_accounts.add(order.account)
                if (
                    order.account.balance < 0
                    and order.account not in self.halted_accounts
                ):
            ...
            price_delta = (
                direction * (price - order.price)
                if hasattr(order, "price")
                else None
            )
            if isinstance(order, MarketOrder):
                execution_price = (
                    best_ask_price if direction > 0 else best_bid_price
                )
                commission = self._get_commission(
                    order.operation, order.amount, execution_price
                )
                ...
                if (
                    price_delta == 0
                    and trade.operation != order.operation
                    and random.random() <= self.order_luck
                ):
                    execution_price = order.price
                    ...
                    if price_delta < 0:
                        ...
                        self.add_order(
                            MarketOrder(
                                ...
                                operation=order.operation,
                                amount=order.amount,
                                time_init=trade_datetime
                                + self.broker_order_delay,
                                time_kill=None,
                            )
                        )
                        del self.orders[id]
                ...
                if (
                    ...
                    and direction * (price - order.target_price) <= 0
                ):
                    self.orders[id] = order._replace(best_price=price)
    ...
    def _order_is_valid(self, order, time: Union[datetime, Arrow]) -> bool:
        if (
            order is None
            or order.account in self.halted_accounts
            or order.time_init is None
        ):
            return False
