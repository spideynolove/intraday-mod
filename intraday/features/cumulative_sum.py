...
class CumulativeSum(Feature):
    def __init__(
        ...
        self.limit = limit
        self.names.append(
            f"cusum_{source}_threshold_{('%.3g' % threshold_factor).replace('.', '_')}_{('%.3g' % threshold).replace('.', '_') if isinstance(threshold, Real) else threshold}"
        )
    ...
    def process(self, frames: Sequence[Frame], state: OrderedDict):
        ...
        expected = (
            getattr(prev_frame, self.source)
            if prev_frame is not None
            else None
        )
        value = getattr(last_frame, self.source)
        self.pos = (
            max(0, self.pos + value - expected) if expected is not None else 0
        )
        self.neg = (
            min(0, self.neg + value - expected) if expected is not None else 0
        )
        sign = 0
        if isinstance(self.threshold, Real):
            ...
            threshold = (
                getattr(last_frame, self.threshold) * self.threshold_factor
            )
