...
class WMASignal(Feature):
    def __init__(
        ...
        self.names = [f"wma_signal_{source}"]
        if write_to in {"state", "both"}:
            self.spaces = OrderedDict(
                {
                    name: gym.spaces.Box(-np.inf, np.inf, shape=(1,))
        ...
        if len(self.prices) < 2:
            ...
            if self.weights is None or len(self.weights) != len(self.prices):
                self.weights = np.arange(
                    1, len(self.prices) + 1, dtype=np.float32
                )
            wma_price = (
                np.sum(np.multiply(self.weights, self.prices))
                / self.weights.sum()
            )
            self.wma_prices.append(wma_price)
            wma_wma_price = (
                np.sum(np.multiply(self.weights, self.wma_prices))
                / self.weights.sum()
            )
            predicted_price = 2 * wma_price - wma_wma_price
            self.predicted_prices.append(predicted_price)
            if self.weights2 is None or len(self.weights2) != len(
                self.predicted_prices
            ):
                self.weights2 = np.arange(
                    1, len(self.predicted_prices) + 1, dtype=np.float32
                )
            wma_predicted_price = (
                np.sum(np.multiply(self.weights2, self.predicted_prices))
                / self.weights2.sum()
            )
