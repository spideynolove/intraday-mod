...
class Clip(Feature):
    def __init__(
        self,
        source: Union[
            Tuple[str, Real, Real], Sequence[Tuple[str, Real, Real]]
        ],
        ...
        for name, minval, maxval in self.source:
            ...
            assert (
                isinstance(minval, Real)
                and isinstance(maxval, Real)
                and minval < maxval
            )
    ...
    def process(self, frames: Sequence[Frame], state: OrderedDict):
        ...
        for i, (name, minval, maxval) in enumerate(self.source):
            ...
            if not isinstance(value, Real):
                warnings.warn(
                    f"{self.__class__.__name__}: Invalid value: {name}={value}"
                )
                result = 0.0
            else:
                result = (
                    minval
                    if value <= minval
                    else maxval if value >= maxval else value
                )
