...
class AbnormalTrades(TradesFeature):
    def __init__(
        ...
        if write_to in {"state", "both"}:
            self.spaces = OrderedDict(
                {
                    name: gym.spaces.Box(0, math.inf, shape=(1,))
    ...
    def update(self, trades: Sequence[Trade]):
        ...
        if (
            self._n_iter >= self.trades_period
            and trade.amount >= self.threshold_factor * self.ema_trade_amount
        ):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(trades_period={self.trades_period}, threshold_factor={self.threshold_factor}, write_to={self.write_to})"
