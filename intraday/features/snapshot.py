...
class Snapshot(Feature):
    ...
    def process(self, frames: Sequence[Frame], state: OrderedDict):
        ...
        price = delta / m if m > 1e-08 else delta
        abs_delta = np.abs(delta)
        d = abs_delta.max() - abs_delta.min()
        proxy = 1.0 - abs_delta / d if d > 1e-08 else 1.0 - abs_delta
        ...
        volume = (
            (v - v.min()) / m
            if m > 1e-08
            else np.ones(self.period, dtype=np.float32)
        )
        trs = self.values[:, 4]
        m = trs.max() - trs.min()
        tr = (
