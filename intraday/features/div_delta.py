...
class DivDelta(Feature):
    def __init__(
        self,
        source: Union[
            Tuple[str, str, Union[str, Real]],
            Sequence[Tuple[str, str, Union[str, Real]]],
        ],
        ...
        for name1, name2, name3 in self.source:
            assert (
                ...
                and isinstance(name2, str)
                and isinstance(name3, (str, Real))
            )
            self.names.append(
                f"divdelta_{name1}_{name2}_{str(name3).replace('.', '_')}"
            )
    ...
    def process(self, frames: Sequence[Frame], state: OrderedDict):
        ...
        for i, (name1, name2, name3) in enumerate(self.source):
            ...
            value3 = (
                getattr(last_frame, name3) if isinstance(name3, str) else name3
            )
            if (
                not isinstance(value1, Real)
                or not isinstance(value2, Real)
                or not isinstance(value3, Real)
                or value3 == 0
            ):
                warnings.warn(
