...
class MoexArchiveProvider(Provider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            date_to is None or date_from is None or date_from <= date_to
        ), "Invalid date_from or date_to"
        self.files = {}
        if isinstance(files, (str, re.Pattern)):
            for file_name in os.listdir(data_dir):
                if isinstance(files, str) and not fnmatch.fnmatch(
                    file_name, files
                ):
                    continue
                elif isinstance(files, re.Pattern) and not files.match(
                    file_name
                ):
                    continue
                ...
                if file_date is None:
                    continue
                if (date_from is None or date_from <= file_date) and (
                    date_to is None or file_date <= date_to
                ):
        ...
        if len(self.files) <= 0:
            raise RuntimeError(
                "Stream files not found for specified file_prefix"
            )
        ...
        self._session_start_datetime: Optional[
            Union[datetime, arrow.Arrow]
        ] = None
        self._session_end_datetime: Optional[Union[datetime, arrow.Arrow]] = (
            None
        )
        self._episode_start_datetime: Optional[
            Union[datetime, arrow.Arrow]
        ] = None
    ...
    def __getitem__(self, index):
        ...
        record_size = (
            self.Stream_Trade_v1_Size
            if self._file_version == 1
            else self.Stream_Trade_v2_Size
        )
        ...
        if self._file_version == 1:
            _, _, _, _, _, dtime_ms, *_ = unpack(
                self.Stream_Trade_v1_Fmt, buffer
            )
        elif self._file_version == 2:
            _, _, _, _, _, dtime_ms, *_ = unpack(
                self.Stream_Trade_v2_Fmt, buffer
            )
        else:
            raise RuntimeError("Invalid file version")
        assert 0 <= dtime_ms <= 24 * 60 * 60 * 1000
        datetime = self._session_start_datetime + timedelta(
            seconds=float(dtime_ms) / 1000.0
        )
        return datetime
    ...
    def reset(
        ...
        if episode_min_duration is None:
            ...
            episode_min_duration = timedelta(
                seconds=float(episode_min_duration)
            )
        ...
        if seek is not None:
            ...
            if seek == "first":
                ...
                if self._file_index is None:
                    raise RuntimeError(
                        "Can not seek to next episode without prior episode"
                    )
                ...
                raise ValueError(
                    'seek can be one of: {"first", "next", "last"}'
                )
            ...
            file_date = date(
                episode_start_datetime.year,
                episode_start_datetime.month,
                episode_start_datetime.day,
            )
            ...
            file_index = (
                rng.randint(len(self.files))
                if rng is not None
                else np.random.randint(len(self.files))
            )
        ...
        self._session_start_datetime, self._session_end_datetime = arrow.get(
            file_date.year,
            file_date.month,
            file_date.day,
            0,
        ...
        if seek is not None:
            ...
            if last_datetime - first_datetime < episode_min_duration:
                raise RuntimeError(
                    f"Session {file_date} can not satisfy episode_min_duration={episode_min_duration}"
                )
            if episode_min_duration.total_seconds() > 0:
                end_index = bisect.bisect_left(
                    self, last_datetime - episode_min_duration
                )
                end_index = min(self._records_count - 1, end_index)
            trade_index = (
                rng.randint(start_index, end_index + 1)
                if rng is not None
                else np.random.randint(start_index, end_index + 1)
            )
            trade_index = max(0, min(self._records_count - 1, trade_index))
            episode_start_datetime = self.__getitem__(trade_index)
            while (
                trade_index > 0
                and self.__getitem__(trade_index - 1) == episode_start_datetime
            ):
                trade_index -= 1
        self._episode_start_datetime = self.__getitem__(trade_index)
        if (
            self.date_from is not None
            and self._episode_start_datetime < self.date_from
            or self.date_to is not None
            and self._episode_start_datetime > self.date_to
        ):
    ...
    def __next__(self) -> NamedTuple:
        ...
        if self._file_version == 1:
            ...
            if not buffer:
                raise StopIteration
            (operation, period, amount, price, id, dtime_ms, open_interest) = (
                unpack(self.Stream_Trade_v1_Fmt, buffer)
            )
        elif self._file_version == 2:
            ...
            if not buffer:
                raise StopIteration
            (operation, period, amount, price, id, dtime_ms, open_interest) = (
                unpack(self.Stream_Trade_v2_Fmt, buffer)
            )
        else:
            raise RuntimeError(
                f"Invalid file format version: {self._file_version}"
            )
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(data_dir={self.data_dir}, date_from={self.date_from}, date_to={self.date_to})"
