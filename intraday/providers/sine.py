...
class SineProvider(Provider):
    def __init__(
        ...
        **kwargs,
        ...
        self.noise_amplitude = math.sqrt(
            amplitude**2 / math.pow(10, SNRdb / 10)
        )
        if isinstance(frequency, float):
            ...
            assert (
                len(frequency) == 2
                and isinstance(frequency[0], Real)
                and isinstance(frequency[1], Real)
            )
            ...
            assert (
                len(period) == 2
                and isinstance(period[0], timedelta)
                and isinstance(period[1], timedelta)
            )
            freq1, freq2 = (
                1 / period[0].total_seconds(),
                1 / period[1].total_seconds(),
            )
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(frequency=({self.freq1}, {self.freq1}), mean={self.mean}, amplitude={self.amplitude}, SNRdb={self.SNRdb}, date_from={self.date_from}, date_to={self.date_to})"
