...
class BinanceKlines(Simulator):
        ...
        **kwargs,
        ...
        if date_to is None:
            ...
            date_to = datetime.combine(
                date_to, datetime.min.time(), tzinfo=timezone.utc
            )
        ...
        d = arrow.get(
            date_from.year, date_from.month, 1, 0, 0, tzinfo=timezone.utc
        )
        while d < date_to:
            ...
            if not os.path.exists(file_path_feather):
                ...
                if not os.path.exists(file_path_zip):
                    self.download_month_archive(
                        symbol, year, month, file_path_zip
                    )
            ...
            if (
                self._first_datetime is None
                or self._first_datetime > first_datetime
            ):
                self._first_datetime = first_datetime
            if (
                self._last_datetime is None
                or self._last_datetime < last_datetime
            ):
    ...
    def convert_month_archive(file_path_zip: str):
        df = pd.read_csv(
            ...
            dtype={
                ...
                "money": np.float32,
                "n_trades": np.int16,
                "buy_volume": np.float32,
                "buy_money": np.float32,
                "ignore": np.float32,
            },
        )
    ...
    def reset(
        ...
        if seek is not None:
            ...
            if episode_start_datetime is None:
                ...
                if self._df is not None and rm < 0.5:
                    ...
                    s, e = (
                        s.timestamp(),
                        (e - episode_min_duration).timestamp(),
                    )
                else:
                    s, e = (
                ...
                episode_start_datetime = arrow.get(
                    episode_start_datetime, tzinfo=timezone.utc
                ).datetime
            ...
            if (
                ...
                and episode_start_datetime < self.date_from
                or self.date_to is not None
                and episode_start_datetime > self.date_to
            ):
                raise ValueError("episode_start_datetime is outside range!")
            file_month = (
                episode_start_datetime.year * 100
                + episode_start_datetime.month
            )
            assert file_month in self.files
            file_index = self.months.index(file_month)
            file_name = self.files[file_month]
        if self._file_index != file_index or not isinstance(
            self._df, pd.DataFrame
        ):
            self.close()
            self._file_index = file_index
            self._df = self.load_month_archive(
        ...
        self._episode_start_datetime = self._df.time_start.iloc[
            record_index
        ].to_pydatetime()
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(data_dir={self.data_dir}, symbol={self.symbol}, spread={self.spread}, date_from={self.date_from}, date_to={self.date_to})"
